...
public class MailService {
    ...
    @Async("mailExecutor")
    public void sendDormantUserMail(String email) {
        try {
            MimeMessage message = generateDormantUserMessage(email);
            sendMessagePort.sendMail(message);
        } catch (MessagingException e) {
            throw new RuntimeException(e);
        }
    ...
    private MimeMessage generateDormantUserMessage(String email) throws MessagingException {
        ...
        String body = "<html><body style='background-color:#ffffff; max-width:600px; margin:0 auto; padding:40px; font-family:sans-serif;'>";
        body += "<h1 style='color:#4D6EF4;'>BAOBAB 휴면 계정 안내</h1>";
        body += "<p style='font-size:16px; line-height:1.6;'>안녕하세요, BAOBAB입니다.<br />";
        body += "회원님께서 최근 1년간 로그인하지 않아 계정이 <b>휴면 상태</b>로 전환되었습니다.<br />";
        body += "다시 서비스를 이용하시려면 로그인 후 휴면 해제를 진행해주세요.</p>";
        body += "<p style='font-size:14px; line-height:1.6;'>언제나 회원님의 편리한 이용을 위해 최선을 다하겠습니다.<br />";
        body += "감사합니다.</p>";
        body += "<p style='font-size:12px; color:#888888; text-align:center; margin-top:40px;'>※ 본 메일은 발신전용입니다. 궁금한 점은 BAOBAB 고객센터로 문의해주세요.</p>";
        body += "</body></html>";
