...
public class ArticleConverter {
    ...
    public ArticleListResponse toResponse(List<Article> articleList) {
        return ArticleListResponse.builder().articles(articleList.stream().map(article -> ArticleInfo.builder().id(article.getId()).title(article.getTitle()).content(article.getContent()).category(article.getCategory()).price(article.getPrice()).registeredAt(article.getRegisteredAt()).status(article.getStatus())
//                    .nickName(article.getUserId())
                .imageList(article.getImageList()).build()).collect(Collectors.toList())).build();
    }
}
